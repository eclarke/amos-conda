          ...
          if test -f "$(DESTDIR)$(bindir)/$$f"  &&  test "$$f" != "$$b"; then \
            ...
            case "$$p" in \
              *.pl) \
                ...
                echo 'use vars qw/$$lpath/;' >> "$(DESTDIR)$(bindir)/$$b"; \
                echo 'BEGIN {' >> "$(DESTDIR)$(bindir)/$$b"; \
                echo "   use Cwd 'abs_path';" >> "$(DESTDIR)$(bindir)/$$b"; \
                echo '   abs_path($$0) =~ /(.*)\//;' >> "$(DESTDIR)$(bindir)/$$b"; \
                echo '   $$lpath = "$$1/../lib/";' >> "$(DESTDIR)$(bindir)/$$b"; \
                echo '}' >> "$(DESTDIR)$(bindir)/$$b"; \
                echo 'use lib $$lpath;' >> "$(DESTDIR)$(bindir)/$$b"; \
              ...
              *.py) \
                ...
                sed -e '1 {s|^#!.*$$||;}' \
                    -e 's|^AMOSDIR[     ]*=.*$$|AMOSDIR=os.path.abspath(sys.path[0]) + os.sep|' \
                    "$(DESTDIR)$(bindir)/$$f" >> "$(DESTDIR)$(bindir)/$$b" \
                || exit 1; \
                ;; \
              *.acf) \
