...
./configure --prefix=/usr/local/AMOS >> /cygdrive/c/cygwin/$1.log 2>&1
if [ $? -ne 0 ]
then
cp /cygdrive/c/cygwin/$1.log /cygdrive/c/cygwin/$1_Failed.log
...
./test_amosvalidate.sh >> /cygdrive/c/cygwin/$1.log 2>&1
if [ $? -ne 0 ]
then
cp /cygdrive/c/cygwin/$1.log /cygdrive/c/cygwin/$1_Failed.log
echo "FAILED: test_amosvalidate.sh" >> /cygdrive/c/cygwin/$1_Failed.log
/usr/bin/expect <<EOD
spawn scp /cygdrive/c/cygwin/$1_Failed.log ssh@sauron.cs.umd.edu:VMlogs
expect "ssh@sauron.cs.umd.edu's password:"
send "123\r"
