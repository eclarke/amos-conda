...
public:
  ...
  : startpos_m(startpos), endpos_m(endpos), node_m(NODECOUNT++), dead(false), startnode (false), endnode(false) {
  ...
  bool isCompressable()
  {
    ...
    if (OPT_DontCompressEnds)
    {
      if (endnode || successor_m[0]->startnode) 
        return false;
    }
...
public:
  ...
  void construct()
  {
    ...
    p->startnode = true; // mark that it is the start of the graph
    ...
    p->endnode = true; // mark it is the end of the graph
  }
...
"\n";
    ...
    tf->getOptions()->addOptionResult("e",   &OPT_DontCompressEnds);
