...
cerr << "Stored node " << i->first << " to be " << contig_bank.lookupIID(i->first) << " in cluster " << i->second << endl;
   ...
   bool changed = true;
   hash_map<uint32_t, double_t, HASHMAP::hash<uint32_t>, HASHMAP::equal_to<uint32_t> > clusterPercents;
         ...
         if (i->second != NULL) {
	       ...
	       edge_bank.fetch(*edges, cte);
               ...
               if (globals.clusters[otherID] != 0 && globals.clusters[otherID] != globals.maxClusterID) {
                  ...
                  if (clusterPercents.find(globals.clusters[otherID]) == clusterPercents.end()) {
                     clusterPercents[globals.clusters[otherID]] = 0;
                  }
