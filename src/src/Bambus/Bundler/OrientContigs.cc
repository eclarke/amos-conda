...
static const int32_t  MAX_TO_LOAD       = 100000;
...
map<AMOS::ID_t, AMOS::ContigEdge_t>* edgeBankCache = NULL;
...
void fetchEdge(ID_t id, Bank_t &edge_bank, ContigEdge_t &cte) {
    if (edgeBankCache != NULL) {
        cte = (*edgeBankCache)[id];
    }
    edge_bank.fetch(id, cte);
}
...
contigOrientation getOrientationByAllEdges(ID_t nodeID, 
   ...
   if (s == NULL || s->size() == 0) {
      ...
      for (set<ID_t, EdgeWeightCmp>::iterator i = s->begin(); i != s->end(); i++) {
        fetchEdge(*i, edge_bank, cte); //edge_bank.fetch(*i, cte);
            ...
            if (visited[i->source] == false) {
               ...
               while (extendPath) {
                  ...
                  for (set<ID_t>::iterator j = ctg2lnk[curr]->begin(); j != ctg2lnk[curr]->end(); j++) {
                     ContigEdge_t cte;
                     fetchEdge(*j, edge_bank, cte);//edge_bank.fetch(*j, cte);
                  ...
                  for (set<ID_t>::iterator j = ctg2lnk[i->source]->begin(); j != ctg2lnk[i->source]->end(); j++) {
                     ...
                     if (cte.getContigs().first == i->source) {
                        ...
                        for (set<ID_t>::iterator k = ctg2lnk[cte.getContigs().second]->begin(); k != ctg2lnk[cte.getContigs().second]->end(); k++) {
                           ...
                           fetchEdge(*k, edge_bank, nextCte);//edge_bank.fetch(*k, nextCte);
...
int main(int argc, char *argv[]) {
   ...
   if (edge_bank.getIDMapSize() < MAX_TO_LOAD) {
       globals.loadEdges = true;
   }
   ...
   if (globals.loadEdges == true) {
       edgeBankCache = new map<ID_t, ContigEdge_t>();
   }
   ...
   map<ID_t, bool> current_nodes_ids;
   ...
   for (vector<ID_t>::iterator iter = contigProcessingOrder.begin(); iter != contigProcessingOrder.end(); iter++) {
      ...
      if (current_nodes_ids.find(ctg.getIID()) == current_nodes_ids.end()) {
...
int max = 0;
         ...
         for (set<ID_t, EdgeWeightCmp>::iterator i = s->begin(); i != s->end(); i++) {
            ...
            if (current_nodes.size() > 0 && current_nodes.top().second > cte.getLinks().size()) {
               if (visitedEdges[cte.getIID()] == 0 && current_nodes_ids.find(otherID) == current_nodes_ids.end()) {
                  nodeToWeight.first = otherID;
                  nodeToWeight.second = cte.getLinks().size();
                  current_nodes.push(nodeToWeight);
                  current_nodes_ids[otherID] = true;
                }
                continue;
             }
...
if (max < cte.getLinks().size()) { max = cte.getLinks().size(); } 
                if (ctg2ort[otherID] != NONE && current_nodes_ids.find(otherID) == current_nodes_ids.end()) {
                  if (globals.debug >=1) {
   ...
   if (globals.compressMotifs == true) {
      ...
      for (AMOS::IDMap_t::const_iterator ci = edge_bank.getIDMap().begin(); ci; ci++) {
          fetchEdge(ci->iid, edge_bank, cte);//edge_bank.fetch(ci->iid, cte);
